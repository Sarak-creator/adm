    ...
    <div className="space-y-6 sm:space-y-8">
        ...
        <div className="space-y-4">
            ...
            <div className={game.hasZoneId ? "sm:col-span-2" : "col-span-1"}>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-[#0a0d18] border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400 transition-all font-display"
                />
              </div>
            </div>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-cyan-300 hover:text-cyan-200 border border-cyan-500/30 font-medium text-xs flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            >
              {isVerifying ? (
                <>
            ...
            {verifiedName && (
              <div className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-emerald-950/80 border border-emerald-500/50 text-emerald-300 shadow-neon-emerald animate-glow-pulse">
                <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                <span className="text-xs">
                  ឈ្មោះគណនី: <strong className="font-bold text-white text-sm ml-1">{verifiedName}</strong>
                </span>
              </div>
            )}
            ...
            {/* Error Message */}
            {verifyError && (
              <div className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-red-950/60 border border-red-500/40 text-red-300 text-xs">
          ...
          <div className="bg-[#0c111e] border border-slate-700 rounded-2xl max-w-lg w-full p-4 sm:p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 text-xs text-slate-300 leading-relaxed">
              <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
                <p className="font-medium text-cyan-300">{game.guideTextKh}</p>
              ...
              <div className="p-3.5 sm:p-4 rounded-xl bg-cyan-950/20 border border-cyan-500/20 text-slate-300">
                ...
                <p className="mt-1">
                  👉 សូមបញ្ចូល <span className="font-mono text-emerald-400">12345678</span> ក្នុងប្រអប់ User ID និង{" "}
                  <span className="font-mono text-emerald-400">2024</span> ក្នុងប្រអប់ Zone ID។
                </p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
          <div className="bg-[#0b101d] border border-cyan-500/30 rounded-2xl sm:rounded-3xl max-w-md w-full p-4 sm:p-6 shadow-2xl relative space-y-4 sm:space-y-5 overflow-hidden max-h-[92vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800">
              ...
              <div className={`font-display font-black text-sm ${timeLeft < 30 ? "text-red-400 animate-bounce" : "text-amber-400"}`}>
                {formatTimer(timeLeft)}
              </div>
            </div>

            {/* QR Code Canvas / Image Display */}
            <div className="relative flex flex-col items-center justify-center p-5 bg-white rounded-2xl shadow-inner mx-auto max-w-[280px]">
            ...
            <div className="space-y-2.5 text-xs">
              <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-900/80 border border-slate-800/80">
                <span className="text-slate-400">ចំនួនទឹកប្រាក់ (Amount):</span>
                <div className="flex items-center gap-2">
              ...
              <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-900/80 border border-slate-800/80">
                <span className="text-slate-400">លេខកូដបញ្ជាទិញ (Order ID):</span>
                <div className="flex items-center gap-2">
            ...
            <div className="flex items-center justify-center gap-2 text-xs text-cyan-400 py-1 font-medium">
      ...
      {completedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fadeIn">
          <div className="bg-[#0b101f] border-2 border-emerald-500/50 rounded-3xl max-w-md w-full p-6 shadow-neon-emerald space-y-5 text-center relative overflow-hidden">
            ...
            {/* Success Icon */}
            <div className="w-16 h-16 rounded-full bg-emerald-500/20 border-2 border-emerald-400 flex items-center justify-center mx-auto text-emerald-400 shadow-neon-emerald">
              <CheckCircle2 className="w-10 h-10 animate-bounce" />
            </div>

            <div>
              <span className="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-xs font-bold font-display uppercase tracking-wider">
                COMPLETED & DELIVERED
              </span>
            ...
            {/* Receipt Box */}
            <div className="p-4 rounded-2xl bg-[#070c17] border border-slate-800 text-xs text-left space-y-2.5">
              <div className="flex justify-between pb-2 border-b border-slate-800">
                <span className="text-slate-400">ហ្គេម (Game):</span>
                <span className="font-bold text-white">{game.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">ឈ្មោះក្នុងហ្គេម (Player):</span>
              ...
              <div className="flex justify-between">
                <span className="text-slate-400">កញ្ចប់ពេជ្រ (Item):</span>
                <span className="font-bold text-cyan-300">
                  {completedOrder.packageName} ({completedOrder.diamondsCount} ពេជ្រ)
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">MooGold Order ID:</span>
                <span className="font-mono text-amber-400 font-bold">
                  {completedOrder.moogoldOrderId || "MG-882391"}
                </span>
              </div>
              <div className="flex justify-between pt-2 border-t border-slate-800">
                <span className="text-slate-400">លេខវិក្កយបត្រ (Ref):</span>
                <span className="font-mono text-slate-300">{completedOrder.orderNumber}</span>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2">
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs flex items-center justify-center gap-2 transition-colors border border-slate-700"
              >
