    ...
    switch (sectionId) {
      ...
      case "ticker": {
        ...
        return (
          <div
            key="ticker"
            className="relative overflow-hidden rounded-xl bg-slate-900/80 border border-cyan-500/20 px-4 py-2 flex items-center gap-3 text-xs"
          >
            <div className="flex items-center gap-1.5 text-amber-400 font-bold flex-shrink-0">
              <Flame className="w-4 h-4 animate-bounce" />
              <span>{landingConfig.ticker.label || "ការទិញចុងក្រោយ (Live Orders):"}</span>
            </div>
            <div className="flex-1 overflow-x-auto whitespace-nowrap text-slate-300 space-x-6 scrollbar-none font-display">
              {items.map((item, idx) => (
                <span key={idx} className="inline-flex items-center gap-1.5">
                  <span className="text-emerald-400">{item}</span>
                  {idx < items.length - 1 && <span className="text-slate-600 ml-4">•</span>}
                </span>
              ))}
            </div>
          </div>
      ...
      case "hero": {
        ...
        return (
          <div
            key="hero"
            className={`relative rounded-3xl overflow-hidden border ${theme.border} bg-[#070b16] p-8 sm:p-10 lg:p-16 ${theme.shadow} min-h-[440px] flex items-center`}
          >
            ...
            <div className="absolute inset-0 z-0">
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[#070b16] via-[#070b16]/90 to-[#070b16]/40" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#070b16] via-transparent to-[#070b16]/50" />
            </div>

            {/* Ambient Glow Effects */}
            <div className={`absolute top-0 right-1/4 w-96 h-96 ${theme.glow1} rounded-full blur-3xl pointer-events-none`} />
            <div className={`absolute bottom-0 left-10 w-80 h-80 ${theme.glow2} rounded-full blur-3xl pointer-events-none`} />

            {/* Hero Content */}
            <div className="relative z-10 max-w-2xl space-y-6">
              {hero.badgeText && (
                <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full ${theme.badgeBorder} text-xs font-semibold backdrop-blur-md`}>
              ...
              <p className="text-sm sm:text-base text-slate-200 leading-relaxed max-w-xl font-medium drop-shadow-sm">
                {hero.description}
              </p>
              ...
              <div className="pt-2 flex flex-wrap items-center gap-4">
                <Link
                  ...
                  className={`px-6 py-3.5 rounded-xl bg-gradient-to-r ${theme.btnGradient} text-slate-950 font-bold text-sm shadow-neon-cyan hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-2`}
                >
                ...
                {hero.secondaryBadgeText && (
                  <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-slate-900/90 border border-slate-800 text-xs text-slate-200 backdrop-blur-md">
              ...
              {hero.trustBadges && hero.trustBadges.length > 0 && (
                <div className="pt-2 flex flex-wrap items-center gap-3 text-xs text-slate-300 font-medium">
                  {hero.trustBadges.map((badge) => (
                    <span
                      key={badge.id}
                      className="inline-flex items-center gap-1.5 text-slate-200 bg-slate-900/80 px-3 py-1.5 rounded-xl border border-slate-800 backdrop-blur-sm"
                    >
                      <DynamicIcon name={badge.icon} className="w-3.5 h-3.5 text-cyan-400" />
      ...
      case "how_it_works": {
        ...
        return (
          <section key="how_it_works" className="rounded-3xl bg-slate-900/40 border border-slate-800 p-8 lg:p-12 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {how.steps.map((step, idx) => (
                <div
                  key={step.id || idx}
                  className="relative p-6 rounded-2xl bg-[#090d1a] border border-slate-800 hover:border-cyan-500/40 transition-all space-y-3 group"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-3xl font-black text-slate-800 group-hover:text-cyan-500/30 transition-colors font-display">
                      {step.stepNumber}
                    </span>
                  </div>
      ...
      case "features": {
        ...
        return (
          <section key="features" className="rounded-3xl bg-slate-900/40 border border-slate-800 p-8 lg:p-12 space-y-8">
            <div className="text-center max-w-2xl mx-auto space-y-2">
              <h2 className="text-2xl lg:text-3xl font-black text-white">
                {why.title}{" "}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-emerald-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {why.features.map((feat, idx) => (
                <div
                  key={feat.id || idx}
                  className="p-6 rounded-2xl bg-[#090d1a] border border-slate-800 space-y-3 hover:border-slate-700 transition-all"
                >
