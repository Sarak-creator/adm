...
export function FAQAccordion({ config }: FAQAccordionProps) {
  ...
  return (
    <section className="rounded-3xl bg-slate-900/40 border border-slate-800 p-8 lg:p-12 space-y-8">
          ...
          return (
            <div
              ...
              className={`rounded-2xl border transition-all overflow-hidden ${
                isOpen
                  ? "bg-[#090d1a] border-cyan-500/50 shadow-neon-cyan/20"
            ...
            >
              <button
                ...
                className="w-full text-left px-5 py-4 flex items-center justify-between gap-4 font-semibold text-sm sm:text-base text-white hover:text-cyan-300 transition-colors"
              >
              ...
              {isOpen && (
                <div className="px-5 pb-5 pt-1 text-xs sm:text-sm text-slate-300 leading-relaxed border-t border-slate-800/60 font-medium">
                  {item.answer}
                </div>
              )}
            </div>
