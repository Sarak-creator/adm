    ...
    switch (sectionId) {
      ...
      case "ticker": {
        ...
        return (
          <div
            key="ticker"
            className="relative overflow-hidden rounded-xl bg-slate-900/80 border border-cyan-500/20 px-3 sm:px-4 py-2 flex items-center gap-2 sm:gap-3 text-[11px] sm:text-xs"
          >
            <div className="flex items-center gap-1.5 text-amber-400 font-bold flex-shrink-0">
            ...
            <div className="flex-1 overflow-x-auto whitespace-nowrap text-slate-300 space-x-4 sm:space-x-6 scrollbar-none font-display">
              {items.map((item, idx) => (
                <span key={idx} className="inline-flex items-center gap-1.5">
                  <span className="text-emerald-400">{item}</span>
                  {idx < items.length - 1 && <span className="text-slate-600 ml-3 sm:ml-4">•</span>}
                </span>
              ))}
            </div>
          </div>
      ...
      case "hero": {
        ...
        return (
          <div
            key="hero"
            className={`relative rounded-2xl sm:rounded-3xl overflow-hidden border ${theme.border} bg-[#070b16] p-5 sm:p-8 lg:p-14 ${theme.shadow} min-h-[360px] sm:min-h-[420px] flex items-center`}
          >
            ...
            <div className="absolute inset-0 z-0">
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[#070b16] via-[#070b16]/95 sm:via-[#070b16]/90 to-[#070b16]/60 sm:to-[#070b16]/40" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#070b16] via-transparent to-[#070b16]/50" />
            </div>
            ...
            <div className="relative z-10 max-w-2xl space-y-4 sm:space-y-6">
              ...
              <h1 className="text-2xl sm:text-4xl lg:text-6xl font-black text-white tracking-tight leading-[1.2] drop-shadow-lg">
                {hero.titlePrefix}{" "}
              ...
              <p className="text-xs sm:text-sm lg:text-base text-slate-200 leading-relaxed max-w-xl font-medium drop-shadow-sm">
                {hero.description}
              </p>
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                ...
                {hero.secondaryBadgeText && (
                  <div className="flex items-center justify-center sm:justify-start gap-2 px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl bg-slate-900/90 border border-slate-800 text-[11px] sm:text-xs text-slate-200 backdrop-blur-md">
              ...
              {hero.trustBadges && hero.trustBadges.length > 0 && (
                <div className="pt-2 flex flex-wrap items-center gap-2 sm:gap-3 text-[11px] sm:text-xs text-slate-300 font-medium">
                  {hero.trustBadges.map((badge) => (
                    <span
                      key={badge.id}
                      className="inline-flex items-center gap-1.5 text-slate-200 bg-slate-900/80 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-xl border border-slate-800 backdrop-blur-sm"
                    >
                      <DynamicIcon name={badge.icon} className="w-3.5 h-3.5 text-cyan-400 flex-shrink-0" />
      ...
      case "how_it_works": {
        ...
        return (
          <section key="how_it_works" className="rounded-2xl sm:rounded-3xl bg-slate-900/40 border border-slate-800 p-5 sm:p-8 lg:p-12 space-y-6 sm:space-y-8">
            <div className="text-center max-w-2xl mx-auto space-y-2">
              ...
              <p className="text-xs sm:text-sm text-slate-400">{how.subtitle}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
              {how.steps.map((step, idx) => (
                <div
                  key={step.id || idx}
                  className="relative p-5 sm:p-6 rounded-2xl bg-[#090d1a] border border-slate-800 hover:border-cyan-500/40 transition-all space-y-2.5 sm:space-y-3 group"
                >
                  <div className="flex items-center justify-between">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center group-hover:scale-110 transition-transform">
                      <DynamicIcon name={step.icon} className="w-5 h-5 sm:w-6 sm:h-6" />
                    </div>
                    <span className="text-2xl sm:text-3xl font-black text-slate-800 group-hover:text-cyan-500/30 transition-colors font-display">
                      {step.stepNumber}
                    </span>
                  </div>
      ...
      case "features": {
        ...
        return (
          <section key="features" className="rounded-2xl sm:rounded-3xl bg-slate-900/40 border border-slate-800 p-5 sm:p-8 lg:p-12 space-y-6 sm:space-y-8">
            <div className="text-center max-w-2xl mx-auto space-y-2">
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-white">
                {why.title}{" "}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-emerald-400">
              ...
              <p className="text-xs sm:text-sm text-slate-400">{why.subtitle}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
              {why.features.map((feat, idx) => (
                <div
                  key={feat.id || idx}
                  className="p-5 sm:p-6 rounded-2xl bg-[#090d1a] border border-slate-800 space-y-2.5 sm:space-y-3 hover:border-slate-700 transition-all"
                >
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center">
                    <DynamicIcon name={feat.icon} className="w-5 h-5 sm:w-6 sm:h-6" />
                  </div>
                  <h3 className="text-sm sm:text-base font-bold text-white">{feat.title}</h3>
      ...
      case "support_banner": {
        ...
        return (
          <section
            key="support_banner"
            className="rounded-3xl relative overflow-hidden bg-gradient-to-r from-cyan-950/70 via-[#0a1124] to-purple-950/70 border border-cyan-500/30 p-8 lg:p-12 shadow-neon-cyan/20"
          >
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="space-y-3 text-center md:text-left max-w-2xl">
                {sup.badge && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-500/20 text-cyan-300 text-xs font-semibold border border-cyan-500/30">
              ...
              <div className="flex-shrink-0">
                <a
                  ...
                  className="px-6 py-3.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-slate-950 font-bold text-sm shadow-neon-cyan flex items-center gap-2 hover:scale-[1.02] active:scale-95 transition-all font-sans"
                >
